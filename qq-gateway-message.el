    ...
    (pcase kind
      ("private"
       (let ((context (qq-gateway-message--private-context message account)))
         ...
         (when-let* ((peer-uid (alist-get 'uid peer)))
           (puthash peer-uid (alist-get 'uin peer)
...
(defun qq-gateway-message--apply-pending-recall (owner normalized merged)
  ...
  (when-let* ((conversation-key
    ...
    (let* ((target (alist-get 'target recall))
           (expected-id (and (equal (alist-get 'kind target) "message")
                             (alist-get 'message_id target)))
           (message-id (alist-get 'server-id merged)))
      (when (and expected-id (not (equal expected-id message-id)))
