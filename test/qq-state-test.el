...
(ert-deftest qq-state-forum-first-page-replaces-legacy-rows-and-deduplicates-by-post-id ()
  (qq-test-with-reset
   (let* ((guild-id "9007199254740993")
          (channel-id "9007199254741999")
          (session-key
          ...
          (post
           `((chat . ((kind . "guild-channel")
                      (guild_id . ,guild-id)
                      (channel_id . ,channel-id)))
             (post_id . "B_synthetic_opaque_post")
     ...
     (puthash session-key
              '(((id . "legacy-row")
                 (server-id . "9007199254742999")
                 (session-key . "guild:9007199254740993:channel:9007199254741999")
                 (time . 1)))
              qq-state--messages-by-session)
     (puthash "9007199254742999" session-key qq-state--message-session-index)
     (qq-state-replace-guild-forum-posts session-key (list post))
     (let ((message (car (qq-state-session-messages session-key))))
       (should (= (length (qq-state-session-messages session-key)) 1))
       (should (equal (alist-get 'id message) "B_synthetic_opaque_post"))
       (should-not (alist-get 'server-id message))
       (should (equal (qq-state-message-anchor message)
                      "B_synthetic_opaque_post"))
       (should (equal (alist-get 'sender-avatar-url message)
                      "https://example.invalid/avatar.png"))
       (should (equal (alist-get 'forum-comment-count message) 3))
       (should (equal (alist-get 'preview message) "Synthetic title"))
       (should-not (gethash "9007199254742999"
                            qq-state--message-session-index)))
     (let ((updated (copy-tree post)))
       (setf (alist-get 'comment_count updated) 4)
       (qq-state-merge-guild-forum-post updated))
     (let ((messages (qq-state-session-messages session-key)))
       (should (= (length messages) 1))
       (should (= (alist-get 'forum-comment-count (car messages)) 4))))))
...
(ert-deftest qq-state-native-recent-accepts-canonical-idless-head ()
  (qq-test-with-reset
   (let* ((message (qq-state-test--native-recent-message))
          (anchor "timeline:slot-a:group:20001:42"))
     (setf (alist-get 'id message nil nil #'eq) anchor
           (alist-get 'server-id message nil nil #'eq) nil
           (alist-get 'canonical-row-key message nil nil #'eq) "42"
           (alist-get 'timeline-class message nil nil #'eq) 'service)
     (qq-state-apply-recent-conversations
      (list (qq-state-test--native-recent-entry :message message)) 1)
     (let ((session (qq-state-session "group:20001")))
       (should (equal (alist-get 'last-message-id session) anchor))
       (should (qq-state-session-recent-p "group:20001")))
     (setf (alist-get 'canonical-row-key message nil nil #'eq) nil)
     (should-error
      (qq-state-apply-recent-conversations
       (list (qq-state-test--native-recent-entry :message message)) 2)))))
