...
(ert-deftest qq-chat-render-preserves-footer-position ()
  (qq-chat-test-with-reset
   ...
   (with-temp-buffer
     ...
     (setq qq-chat--session-key "private:10001")
     (qq-chat-render)
     (qq-chat--set-reply-message '((server-id . "42")
                                   (raw-message . "hello from footer")))
     (qq-chat--update-frame-preserving-point)
     (goto-char (point-min))
     (search-forward "Replying to 42")
     (beginning-of-line)
     (let ((before (point)))
       (qq-chat-render)
       (should (= before (point)))
...
(ert-deftest qq-chat-timeline-hides-recalled-messages-by-default ()
  "Like telega default: recalled rows leave the timeline."
  (qq-chat-test-with-reset
